/*
	...
	Changelog:
	* poker function: changed display to wrap text
	* colourMode: adjusted display to game names instead of colour names
	* colourMode: added 50 ms delay after quick deal
	* manualMode: clears the display at the start of the function
	* main: clears the display when returns to main menu
*/
...
{
	const int DIST_BW_PILES = 10; // set distance between each pile (in cm)
...
void colourMode()
{
	...
	while (SensorValue[S3] != (int)colorGreen && SensorValue[S3] != (int)colorBlue && SensorValue[S3] != (int)colorRed && SensorValue[S3] != (int)colorYellow)
	{}
